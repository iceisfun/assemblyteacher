...
It explains why a call in tail position becomes a `jmp` (the compiler tears the
current function's frame back down and jumps without pushing a return address, so
the callee returns directly to the original caller — constant stack space for tail
recursion), traces how that makes the intermediate function vanish
